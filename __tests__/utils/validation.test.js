...
describe("validateRegister", () => {
    it("returns valid for correct username and password", () => {
    ...
    it("trims username and password", () => {
        const result = validateRegister("  user12  ", "  pass1234  ");
        expect(result.valid).toBe(true);
    ...
    it("rejects username shorter than 5 characters", () => {
        const result = validateRegister("user", "password123");
    ...
    it("rejects username longer than 30 characters", () => {
        const result = validateRegister("a".repeat(31), "password123");
        expect(result.valid).toBe(false);
        expect(result.errors).toContain(`Username must be at most ${USERNAME_MAX} characters.`);
    });

    it("rejects username with special characters", () => {
        const result = validateRegister("user@name", "password123");
        expect(result.valid).toBe(false);
    ...
    it("allows username with letters, numbers, and underscores", () => {
        expect(validateRegister("user_123", "password123").valid).toBe(true);
        expect(validateRegister("User123", "password123").valid).toBe(true);
    });

    it("rejects empty username", () => {
        const result = validateRegister("", "password123");
        expect(result.valid).toBe(false);
        expect(result.errors).toContain("Username is required.");
    });

    it("rejects password shorter than 8 characters", () => {
        const result = validateRegister("validuser", "short");
        expect(result.valid).toBe(false);
        expect(result.errors).toContain(`Password must be at least ${PASSWORD_MIN} characters.`);
    });

    it("rejects password longer than 128 characters", () => {
        const result = validateRegister("validuser", "a".repeat(129));
        expect(result.valid).toBe(false);
    ...
    it("returns multiple errors when both username and password invalid", () => {
        const result = validateRegister("ab", "x");
        expect(result.valid).toBe(false);
        expect(result.errors.length).toBeGreaterThanOrEqual(2);
    });

    it("handles null/undefined username and password", () => {
        expect(validateRegister(null, "password123").valid).toBe(false);
        expect(validateRegister(undefined, "password123").valid).toBe(false);
        expect(validateRegister("validuser", null).valid).toBe(false);
    });
});
...
describe("validateLogin", () => {
    ...
    it("returns invalid when username is empty", () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.message).toBe("Please enter both username and password.");
    });
